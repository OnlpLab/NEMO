...
1. For the full please consult the inline documentation at the end of `nemo.py`. 
...
1. NCRFpp was great for our experiments on the NEMO corpus (which is given, constant, data), but it holds some caveats for real life scenarios of arbitrary text:
    * fastText is not used on the fly to obtain vectors for OOV words (which were not seen in our Wikipedia corpus). Instead, it is used as a regular embedding matrix. In our experiments we created such a matrix in advance with all the words of our , and used during training. Hence the full generalization capacities of fastText shown in our experiments, are not available in the currently provided models, which will perform slightly worse than they could on arbitrary text.  
    * For optimal performance on your data, you're probably better off retraining with an (you can find our training configs and hyper parameters in our paper and experiment code).
    * Finally, We currently do not provide an API, only file input/outputs. 
1. In the near future we plan to publish a new cleaner end-to-end implementation. 
